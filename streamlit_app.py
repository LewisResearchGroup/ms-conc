...
def display_button():
    ...
    if display_instructions == 'Show Instructions':
        st.markdown("""
         ...
         #####    2) Next, upload the data file from Mint or Maven.  You may upload the full results or dense peak max data file generated from Mint. You do not have to remove or re-arrange any columns generated by these programs before uploading. Follow the links to find peaklist file templates.
         ...
         ######     Please make sure that the file names for standards and compound names in the standards concentration file and peaklist data file are the same. Remove any compounds that are not in both the standards concentration file and peaklist data file.
         """)
        st.write('''         
         #####    3) When the standards concentrations file and peaklist data files are uploaded, a table with the parameters of the standard curves (slope, intercept, upper limit of quantification, lower limit of quantification) will be generated automatically and includes:
         ###### Peak_label\n
         ###### Log_scale_slope\n
         ###### Log_scale_intercept\n
         ...
         #####    5) At the bottom of this page, you can visualize the standard curves and the linear ranges predicted for each compound (black dots). Concentrations outside the linear range are shown in grey.    
...
#          ''')
# state = AS.AppState()
st.sidebar.write( '## 1) Please upload standard concentrations file' )
st.sidebar.write("a sample file can be found [here](https://github.com/LSARP/ms-conc/tree/main/sample_files)")

std_info = st.sidebar.file_uploader('upload standard concentrations file')
...
try:
    ...
    st.write('## Please upload your information file to start')
...
try:
    ...
    st.write('## your metabolomics data file:')
